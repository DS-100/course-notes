...
Like with expectation, **variance and standard deviation are numbers, not random variables**! Variance helps us describe the variability of a random variable and can be viewed as the chance error between a sample $X$ and the population mean.
...
To compute the variance, we will use the computational formula. We first find that:
