...
### The Gradient Vector
As we learned earlier, we can take the derivative of the loss function to determine the minimum value for our parameters. On a 2D (or higher) surface, the best way to go down (gradient) is described by a *vector*.

For the *vector* of parameter values $\vec{\theta} = \begin{bmatrix}
           \theta_{0} \\
           \theta_{1} \\
         \end{bmatrix}$, we have a two parameter model (d = 2). To calculate our gradient vector, we can take the *partial derivative* of loss with respect to each parameter: $\frac{\partial L}{\partial \theta_0}$ and $\frac{\partial L}{\partial \theta_1}$. 
...
Let's now dive deeper into gradient and stochastic gradient descent. In the previous lecture, we discussed how finding the gradient across all the data is extremeley computationally taxing.
...
Computing $(\mathbb{X}^{\top}\mathbb{X})^{-1}$ takes $O(nd^2) + O(d^3)$ time while $\mathbb{X}^{\top}\mathbb{Y}$ takes $O(nd)$ time where $n$ is the number of samples and $d$ is the number of features. We can see that the first term clearly dominates the complexity — this can be problematic for high-dimensional models and very large datasets.

Looking at the time complexity for gradient descent, we can see it takes $O(nd)$ time. 

<center><img src="images/complexity_grad_descent.png" alt='complexity_grad_descent' width='600'></center>

Now suppose we run $T$ iterations. The final complexity would then be $O(Tnd)$. Typically, $n$ is much larger than $T$ and $d$. How can we reduce the cost of this algorithm using a technique from Data 100? Do we really need to use $n$ data points? 

We don't! Instead, we can use stochastic gradient descent.
...
Let's use a sample of our data to approximate the true gradient of the loss surface. Say we sample $b$ records ($s_1, \cdots, s_b$) of our $n$ datapoints. Our new (stochastic) gradient descent function will be $\nabla_{\vec{\theta}} L (\vec{\theta^{(t)}}) = \frac{1}{b}\sum_{i=1}^{b}\nabla_{\vec{\theta}} l(y_{s_i}, f_{\vec{\theta}^{(t)}}(X_{s_i}))$ and will now have a time complexity of $O(bd)$. Much faster! 
...
But how do we decide our mini-batch size ($b$), or the number of datapoints in our sample? The original stochastic gradient descent algorithm set $b=1$. Although we don't use such a small mini-batch size often, $b$ typically is small. When choosing $b$, there are several factors to consider. A larger batch size results in a better gradient estimate, parallelism, and other systems factors. But there are diminishing returns — there is a tradeoff between batch size and time complexity after all. On the other hand, a smaller batch size will be faster and have more frequent updates.
...
### Bonus Material: Stochastic Gradient Descent in `PyTorch`

While this material is out of scope for Data 100, it is useful if you plan to enter a career in data science!

In practice, when computing gradients and implementing gradient descent, you will use software packages such as `PyTorch`. As outlined below, you'll follow three main steps:

1. Sample a batch of the data.
2. Compute the loss and the gradient.
3. Update your gradient until you reach an appropriate estiamte of the true gradient.

<center><img src="images/pytorchsgd.png" alt='pytorch_sgd' width='400'></center>
