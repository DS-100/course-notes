---
title: Feature Engineering (Old Notes from Fall 2024)
execute:
  echo: true
  warning: false
format:
...
But how do we decide our mini-batch size ($b$), or the number of datapoints in our sample? Mini-batch size is typically **small**. 

- The original **Stochastic Gradient Descent** algorithm uses $\boldsymbol{b=1}$ so that only one sample is used to approximate the gradient at a time. 
- When choosing $b$, there are several factors to consider: 
  - **Larger b** $\rightarrow$ better **gradient estimates**, **parallelism**, and other **systems factors**, but also rapidly diminishing returns with more complexity
  - **Smaller b** $\rightarrow$ **faster** and has **more frequent updates**
