...
TSV files can be loaded into `pandas` using `pd.read_csv()` with the `delimiter` parameter: `pd.read_csv("file_name.tsv", delimiter="\t")`. 

```{python}
#| echo: false
pd.read_csv('data/elections.tsv', sep="\t").head(3)
```
...
Below are some potential solutions to common issues:
