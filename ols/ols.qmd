...
<img src="images/observation.png" alt='observation' width='400'>
...
Here, $\mathbb{\hat{Y}}$ is the **prediction vector** with dimensions $(n \times 1)$. It contains the prediction made by the model for each of the $n$ input observations.
...
Note that for OLS with an intercept term (e.g., $$\hat{y} = \theta_0 + \theta_1x_1 + \theta_2x_2 + \vdot + \theta_px_p) $\mathbb{R}^2$ is equal to the square of the correlation between $y, \hat{y}$. On the other hand, for SLR  $\mathbb{R}^2 = r^2$, the correlation between $x, y$. The proof of these last two properties is out of scope for this course.
