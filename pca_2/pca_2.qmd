...
Using calculus and optimization techniques (take EECS 127 if you're interested!), we we find that this loss is minimized when 
$$Z = XW^T$$ 
...
1. $w$ is a **unitary eigenvector** of the covariance matrix $\Sigma$. In other words, it's norm is equal to 1: $||w||^2 = ww^T = 1$
2. The loss is minimized when $w$ is the eigenvector with the **largest eigenvalue** $\lambda$.
