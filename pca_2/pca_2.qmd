...
**Do legislators' roll call votes show a relationship with their political party?** Let's use PCA to find out!
...
Exploring the data, we can see that the class names are:
...
We have loaded in a lot of data (feel free to play around with it later —— try building a classifier!). But for the purposes of this demo, let's take a small sample of the training data.
...
Let's break this down further and look at it by class:
...
While visualizing 20 images is doable, this dataset contains 60,000 images across the 10 fashion categories. So how would we visualize the entire dataset? Let's use PCA to find a low-dimensional representation of the images.
...
We can then "unroll" the pixels into a single row vector containing 28*28 = 784 dimensions.
...
Now, let's center our data.

```{python}
X = X - X.mean(axis=0)
```

Finally, we can run PCA, this time using `sklearn`.
...
Let's see what we found from conducting PCA!
...
Looking at the scree plot above, we can see that the line starts flattening out around 2 or 3. This indicates that most of the data is explained in the first two or three dimensions.

### Applying PCA to a Subset of the Data

Now, we'll work with a subset of the data corresponding to tougher images, particularly those in the "Coat" and "Pullover" classes. Let's see if we can build a better embedding for this data.
...
Going through the same steps we did above to conduct PCA, we end up with the following plot. Here, the red points reflect data in the "Pullover" class, and the blue points correspond to data in the "Coat" class.
