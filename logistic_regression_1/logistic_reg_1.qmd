...
The least squares approach is technically a *valid* model, but it is not *good*. The linear model's predictions can be outside of the label range {$0$, $1$}, which doesn't make sense. It is also sensitive to outliers -- that is, if we found a data point far from these two clusters, the least squares line would  be significantly worse.

Instead, what if used a non-linear model that was a better fit of our data, and bounded predictions between [$0$, $1$]? We can look to the Data 8 Graph of Averages to formulate this model.
