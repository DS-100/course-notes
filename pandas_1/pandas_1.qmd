...
## Index, Columns, and Shape

On the other hand, column names in `pandas` are almost always unique. Looking back to the `elections` dataset, it wouldn't make sense to have two columns named "Candidate".
...
### Indexing with .loc: label-based extraction
...
For example, to select a single value, we can select the the row labeled `0` and the column labeled `Candidate` from the `elections` DataFrame.
