...
Data scientists work with data stored in a variety of formats. The primary focus of this class is in understanding tabular data - one of the most commonly used formats used in data science. Tabular data contain large amounts of data that are stored in rows and columns. This note introduces DataFrames, which are one of the most popular ways to represent these data tables. We’ll also introduce `pandas`, the standard Python package for manipulating data in DataFrames.
...
```{python}
elections.set_index("Candidate", inplace=True) # This sets the index of the elections table to the "Candidate" column
```
...
```{python}
elections.reset_index(inplace=True) # This resets the index to be the numbers from 0 to 181
```
