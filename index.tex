...
\begin{document}
\maketitle
\ifdefined\Shaded\renewenvironment{Shaded}{\begin{tcolorbox}[boxrule=0pt, enhanced, borderline west={3pt}{0pt}{shadecolor}, breakable, frame hidden, sharp corners, interior hidden]}{\end{tcolorbox}}\fi
...
\begin{tcolorbox}[enhanced jigsaw, breakable, opacitybacktitle=0.6, rightrule=.15mm, colbacktitle=quarto-callout-note-color!10!white, colframe=quarto-callout-note-color-frame, coltitle=black, titlerule=0mm, bottomrule=.15mm, colback=white, opacityback=0, bottomtitle=1mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, toptitle=1mm, left=2mm, toprule=.15mm, leftrule=.75mm, arc=.35mm]
...
\midrule
52753  &  1965 &    Candis &      6 \\
201586 &  2012 &   Valarie &      5 \\
99754  &  1986 &      Leah &    303 \\
170032 &  2005 &    Leslye &     16 \\
377559 &  2013 &  Maxximus &      5 \\
\bottomrule
\end{tabular}
...
{} &  Year &    Name &  Count \\
\midrule
340940 &  2000 &  Daylin &      5 \\
339636 &  2000 &  Lonnie &     18 \\
340788 &  2000 &   Niles &      6 \\
149819 &  2000 &  Brandi &     43 \\
\bottomrule
\end{tabular}
...
\begin{verbatim}
<pandas.core.groupby.generic.DataFrameGroupBy object at 0x7f9dc8e252e0>
\end{verbatim}
...
Example Questions: - What colors should we use? - How wide should the
bars be? - Should the legend exist? - Should the bars and axes have dark
borders?

To accomplish goal 2, here are some ways we can improve plot: -
Introducing different colors for each bar - Including a legend -
Including a title - Labeling the y-axis - Using color-blind friendly
palettes - Re-orienting the labels - Increase the font size
...
we get a histogram that appears bimodal. - Density curve reinforces
belief in this bimodality.
...
something interesting here worth pursuing. - Deeper analysis necessary!
...
\texttt{Maternal\ Pregnancy\ Weight} and \texttt{Birth\ Weight} colums,
using both \texttt{matplotlib} and \texttt{seaborn}.
...
\NormalTok{plt.scatter(births[}\StringTok{\textquotesingle{}Maternal Pregnancy Weight\textquotesingle{}}\NormalTok{], births[}\StringTok{\textquotesingle{}Birth Weight\textquotesingle{}}\NormalTok{])}
\NormalTok{plt.xlabel(}\StringTok{\textquotesingle{}Maternal Pregnancy Weight\textquotesingle{}}\NormalTok{)}
...
\NormalTok{sns.scatterplot(data }\OperatorTok{=}\NormalTok{ births, x }\OperatorTok{=} \StringTok{\textquotesingle{}Maternal Pregnancy Weight\textquotesingle{}}\NormalTok{, y }\OperatorTok{=} \StringTok{\textquotesingle{}Birth Weight\textquotesingle{}}\NormalTok{,}
...
\texttt{Maternal\ Pregnancy\ Weight} and \texttt{Birth\ Weight} in both
maternal smokers and non-smokers. If we wish to see the relationship's
strength more clearly, we can use \texttt{sns.lmplot}.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{sns.lmplot(data }\OperatorTok{=}\NormalTok{ births, x }\OperatorTok{=} \StringTok{\textquotesingle{}Maternal Pregnancy Weight\textquotesingle{}}\NormalTok{, y }\OperatorTok{=} \StringTok{\textquotesingle{}Birth Weight\textquotesingle{}}\NormalTok{, }
...
We can make out a weak, positive relationship in pregnancy weight and
birth weight for both maternal smokers and non-smokers (slightly more
positive in maternal smokers).
...
\begin{verbatim}
<seaborn.axisgrid.JointGrid at 0x7faa9c30ac10>
\end{verbatim}

\begin{figure}[H]

{\centering \includegraphics{visualization_2/visualization_2_files/figure-pdf/cell-10-output-2.png}
...
\begin{verbatim}
<seaborn.axisgrid.JointGrid at 0x7faa9c42b820>
\end{verbatim}
...
\subsection{Censuses}\label{censuses}}
...
\hypertarget{u.s.-decennial-census}{%
\subsubsection{U.S. Decennial Census}\label{u.s.-decennial-census}}

Was held in April 2020. Counts every person living in all 50 states, DC,
and US territories. (Not just citizens.) Mandated by the Constitution.
Participation is required by law. Important uses: - Allocation of
Federal funds. - Congressional representation. - Drawing congressional
and state legislative districts.

\begin{figure}

{\centering \includegraphics{sampling/images/census_map.PNG}
...
What is asked, and how it is asked, can affect: - How the respondent
answers. - Whether the respondent answers.

\hypertarget{sampling-definitions}{%
\section{Sampling: Definitions}\label{sampling-definitions}}
...
\item
  ...
  \tightlist
  \item
    \textbf{chance error}: random samples can vary from what is
    expected, in any direction.
  \item
    \textbf{bias}: a systematic error in one direction.
  \end{itemize}
\end{itemize}

\hypertarget{population-sample-and-sampling-frame}{%
\subsection{Population, Sample, and Sampling
Frame}\label{population-sample-and-sampling-frame}}

\textbf{Population}: The group that you want to learn something about.

\textbf{Sampling Frame}: The list from which the sample is drawn. - If
you're sampling people, the sampling frame is the set of all people that
could possibly end up in your sample.

\textbf{Sample}: Who you actually end up sampling. - A subset of your
sampling frame.
...
\hypertarget{other-kinds-of-populations}{%
\subsection{Other Kinds of
Populations}\label{other-kinds-of-populations}}

The individuals in a population are not always people!

Could be: - Bacteria in your gut (sampled using DNA sequencing) - Trees
of a certain species - Small businesses receiving a microloan -
Published results in a journal / field

In any of these cases we might examine a sample and try to draw an
inference about the population it came from. Simplest example: what \%
have some binary property (like voting intention)?

\hypertarget{bias-a-case-study}{%
\section{Bias: A Case Study}\label{bias-a-case-study}}

In 1936, President Franklin D. Roosevelt went up for re-election against
Alf Landon. As is usual, polls were conducted in the months leading up
to the election to try and predict the outcome.

\hypertarget{the-literary-digest-election-prediction}{%
\subsection{The Literary Digest: Election
Prediction}\label{the-literary-digest-election-prediction}}

The Literary Digest was a magazine. They had successfully predicted the
outcome of 5 general elections coming into 1936.

They sent out their survey to 10,000,000 individuals, who they found
from: - Phone books. - Lists of magazine subscribers. - Lists of country
club members.

\begin{figure}

{\centering \includegraphics{sampling/images/electionresults.PNG}

}

\caption{Election\_results1}

\end{figure}

\hypertarget{the-literary-digest-what-happened}{%
\subsection{The Literary Digest: What
Happened}\label{the-literary-digest-what-happened}}

The Literary Digest sample was not representative of the population. The
Digest's \textbf{sampling frame} included: - people in the phonebook,
subscribed to magazines, and went to country clubs. - These people were
more affluent and tended to vote Republican (Landon).

Only 2.4 million people actually filled out the survey! - 24\% response
rate (low). - Who knows how the 76\% \textbf{non-respondents} would have
polled?

\hypertarget{gallups-poll-election-prediction}{%
\subsection{Gallup's Poll: Election
Prediction}\label{gallups-poll-election-prediction}}

George Gallup, a rising statistician, also made predictions about the
1936 elections. His estimate was much closer despite having a smaller
\textbf{sample size} of ``only'' 50,000 (still more than necessary).

George Gallup also predicted what The Literary Digest was going to
predict, within 1\%, with a sample size of only 3000 people. He
predicted the Literary Digest's \textbf{sampling frame} (phonebook,
magazine subscribers, country clubs). So he sampled those same
individuals!

\begin{figure}

{\centering \includegraphics{sampling/images/electionresults2.PNG}

}

\caption{Election\_Results2}

\end{figure}

Samples, while convenient, are subject to chance error and
\textbf{bias}. Election polling, in particular, can involve many sources
of bias: - Who responds to polls? - Do voters tell the truth? - How can
we predict turnout?
...
a bad sampling method.

Our main goal is to gather a sample that is representative of the
population it came from. The most common way to accomplish this is by
randomly sampling from the population.

\hypertarget{convenience-sample}{%
\subsection{Convenience Sample}\label{convenience-sample}}

A \textbf{convenience sample} is whatever you can get ahold of.

Example: Scientists in New South Wales (AUS) collect specimens from
eucalyptus trees to keep in museums, recording \textbf{where they came
from} in lat / long.

Can we use this data to map the geographic distribution of eucalyptus
trees?

Warning: - Haphazard sample ≠ random sample. - Many potential sources of
bias!

\hypertarget{probability-sample}{%
\subsection{Probability Sample}\label{probability-sample}}

Why sample at random? \textbf{Not} just to eliminate bias!

Random samples \textbf{do not} always produce \textbf{unbiased
estimates} of population quantities. - Sometimes \textbf{close} to
unbiased (e.g.~sample median for population median) With random samples
we can \textbf{estimate} the bias and chance error, which helps us
quantify uncertainty.

In a probability sample we know the chance any given set of individuals
will be in the sample. All individuals in the population need not have
the same chance of being selected.

The real world is usually more complicated!

For example: - When Gallup calls, most people don't answer - We don't
know the probability a given bacterium will get into a microbiome sample
- We don't know journals' publication process

If the sampling / measurement process isn't fully under our control, we
try to model it.

\hypertarget{common-random-sampling-schemes}{%
\subsection{Common Random Sampling
Schemes}\label{common-random-sampling-schemes}}

A \textbf{random sample with replacement} is a sample drawn
\textbf{uniformly} at random \textbf{with} replacement. - Random doesn't
always mean ``uniformly at random,'' but in this specific context, it
does. - Some individuals in the population might get picked more than
once

A simple random sample (SRS) is a sample drawn uniformly at random
without replacement. - Every individual (and subset of individuals) has
the same chance of being selected. - Every pair has the same chance as
every other pair. - Every triple has the same chance as every other
triple. - And so on.

\hypertarget{example-scheme-1-stratified-random-sample}{%
\subsection{Example Scheme 1: Stratified random
sample}\label{example-scheme-1-stratified-random-sample}}

Consider an example such as Gotham City, where about 50\% of the
population is male-identifying. We are interested in how Gotham City
votes (Democratic, Republican) in some election. We want to choose 100
voters for a survey: - By sampling uniformly at random (SRS) from
male-identifying individuals in Gotham, we choose exactly 50 individuals
- We then uniformly sample (SRS) another 50 from non-male-identifying
individuals

This is a \textbf{probability sample} (Consider why we might want to do
this?)

For any group of 100 people: - If there are not exactly 50
male-identifying individuals, the group cannot be chosen - Any other
group: chance is of choosing it is 1 / \# of such groups

Replace 50/50 with 80/20 (80 male-identifying, 20 non-male-identifying)
- Would it still be a probability sample? - Would the \% Dem in the
sample be unbiased?

Challenge: how could we make it unbiased?

\hypertarget{a-very-common-approximation-for-sampling}{%
\subsection{A Very Common Approximation For
Sampling}\label{a-very-common-approximation-for-sampling}}
...
As seen here, probabilities of sampling with replacement are much easier
to compute and lead to a very good approximation when the population
size is large!

\hypertarget{up-to-this-point}{%
\subsection{Up to This Point}\label{up-to-this-point}}

\begin{figure}

{\centering \includegraphics{sampling/images/sampling.PNG}

}

\caption{sampling\_diagram}

\end{figure}

If a sample was \textbf{randomly sampled with replacement} from the
population: - It is a probability sample. - We can quantify error and
bias (to be covered later). - \textbf{Given the population
distribution}, we can compute the probability of us getting a
\textbf{particular sample}.

Note: In practicality, we almost \textbf{never} know the population
distribution! But this is a good start.
...
the data to understanding the world

Without knowing / assuming something about how the data were collected:
- There is no connection between the sample and the population. - The
data set doesn't tell us about the world behind the data.
