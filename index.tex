...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, breakable, rightrule=.15mm, colframe=quarto-callout-note-color-frame, colbacktitle=quarto-callout-note-color!10!white, leftrule=.75mm, toptitle=1mm, arc=.35mm, opacityback=0, toprule=.15mm, bottomtitle=1mm, left=2mm, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Note}, bottomrule=.15mm, opacitybacktitle=0.6, titlerule=0mm, colback=white]
...
Imagine you have collected or have been given a box of data. What do you
do next?

The first step is to clean your data (\textbf{data cleaning}). Data
scientists have coined the term \textbf{exploratory data analysis (EDA)}
to describe the process of transforming raw data to insightful
observations.

EDA is an \emph{open-ended} analysis of transforming, visualizing, and
summarizing patterns in data. In order to conduct EDA, we first need to
familiarize ourselves with an important programming tool:
\texttt{pandas}.
...
\subsubsection{Indexing with .loc}\label{indexing-with-.loc}}
...
\subsubsection{Indexing with .iloc}\label{indexing-with-.iloc}}
...
\subsubsection{Indexing with {[}{]}}\label{indexing-with}}
...
\paragraph{A slice of row numbers}\label{a-slice-of-row-numbers}}
...
\paragraph{A list of column labels}\label{a-list-of-column-labels}}
...
\paragraph{A single column label}\label{a-single-column-label}}
