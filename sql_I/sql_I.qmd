...
To start, we'll look at a database called `basic_examples.db` and connect to it using SQLite.
...
By default, `ORDER BY` will display results in ascending order with the lowest values at the top of the table, `ASC`. To sort in descending order, we use the `DESC` keyword after specifying the column to be used for ordering.
