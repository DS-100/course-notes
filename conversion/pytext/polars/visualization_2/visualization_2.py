...
#
# ```{image} images/distribution_summary.png
# :alt: Illustration of what plots are appropriate based on the variables being used.
# :width: 700px
# ```
#
# #### Scatter Plots
...
# %% id="c6ce4f19"
#| fig-alt: Female adult literacy against percent growth broken out by continent. 
...
# %% id="359b423a"
#| fig-alt: Female adult literacy against percent growth with jittering applied and the size of datapoints changed.
...
# %% id="68631fd3"
#| fig-alt: Female adult literacy against percent growth with a linear regression line overlaid.
sns.lmplot(data = wb, x = "per capita: % growth: 2016", \
           y = "Adult literacy rate: Female: % ages 15 and older: 2005-14")

plt.title("Female adult literacy against % growth");

# %% [markdown] id="e8206201"
...
# %% id="e837690a"
#| fig-alt: Female adult literacy against percent growth joint plot. Both a scatterplot, a histogram for the x values, and a histogram for the y values are shown.
...
# %% id="a889e346"
#| fig-alt: Female adult literacy against percent growth jointplot with two histograms and instead of a scatterplot, a hex plot.
sns.jointplot(data = wb, x = "per capita: % growth: 2016", \
              y = "Adult literacy rate: Female: % ages 15 and older: 2005-14", \
              kind = "hex")
...
# %% id="98f6fcae"
#| fig-alt: Female adult literacy against percent growth contour plot.
...
# :open: false
# ```python
# #| fig-alt: Adult literacy rate against GNI per capita
#
...
#
# ```{image} images/linearize.png
# :alt: The original data is plotted on the left with a clear non-linear relationship. On the right, the data has been transformed via log and a line is drawn on top of the data.
# :width: 800px
# ```
#
...
# %% id="9a165feb"
#| fig-alt: Adult literacy rate against Log(GNI per capita)
...
# %% id="dd69b8a5"
#| fig-alt: Apply a log transformation to the x values and a power transformation to the y values
...
# :open: false
# ```python
# #| fig-alt: The untransformed data and the modeled relationship.
...
#
# - `plotly` makes it very easily creates interactive plots.
...
#
# ```{image} images/rugplot_encoding.png
# :alt: A dataframe is shown with an arrow pointing to lines, one per row labeled Mark (represents a datum). From there, another arrow points to values like 10x, 15px, and NONE labeled Encoding (Maps datum to visual position). Finally, a last arrow points to a rug plot.
# :width: 600px
# ```
#
...
# - Color
#
# ```{image} images/mutli_dim_encodings.png
# :alt: Example plot using four dimensions of the data.
# :width: 400px
# ```
#
...
#
# ```{image} images/wrong_scale_viz.png
# :alt: Two lines representing two differnt things, cancer screenings and abortions are shown overlapping with each other.
# :width: 350px
# ```
#
...
#
# ```{image} images/good_viz_scale_1.png
# :alt: Now you can see that the cancer screen line is way above the line for abortions
# :width:345
# ```
#
# ```{image} images/good_viz_scale_2.png
# :alt: Another way to visualize the percent change in the number of procedures, this time with a bar chart.
# :width:345
# ```
#
#
...
#
# ```{image} images/unrevealed_viz.png
# :alt: Line plot showing COVID-19 cases from March 2020 to April 2021.
# :width:345
# ```
#
# ```{image} images/revealed_viz.png
# :alt: The same data is shown, this time zoomed in on March 2020 to May 2020.
# :width:345
# ```
#
...
#
# **Jet Colormap:**
# ```{image} images/jet_colormap.png
# :alt: Jet Colormap with vivid colors
# :width: 250px
# ```
#
# **Viridis Colormap**
# ```{image} images/viridis_colormap.png
# :alt: Viridis Colormap with less bright colors.
# :width: 250px
# ```
#
...
#
# ```{image} images/jet_four_by_four.png
# :alt: Four different color palettes are used to show the same four sets of data.
# :width: 300px
# ```
#
# ```{image} images/jet_3_images.png
# :alt:Comparison of black and white with Jet.
# :width: 300px
# ```
#
# The first diagram compares how a variety of colormaps represent pixel data that transitions from a high to low intensity. These include the jet colormap (row a) and grayscale (row b). Notice how the grayscale images do the best job in smoothly transitioning between pixel data. The jet colormap is the worst at this - the four images in row (a) look like a conglomeration of individual colors. 
#
# The difference is also evident in the images labeled (a) and (b) in the first diagram. The grayscale image is better at preserving finer detail in the vertical line strokes. Additionally, grayscale is preferred in X-ray scans for being more neutral. The intensity of the dark red color in the jet colormap is frightening and indicates something is wrong.
#
...
# ```{image} images/jet_perceptually_uniform.png
# :alt: Jet colormap is perceived as a piecewise linear function. 
# :width:300
# ```
#
# **Viridis Colormap Perception**
# ```{image} images/viridis_perceptually_uniform.png
# :alt: Viridis colormap is perceived as a straight line.
# :width: 336
# ```
#
...
#
# ```{image} images/markings_viz.png
# :alt: Examples of markings.
# :width: 400
# ```
#
...
#
# ```{image} images/male_female_earnings_barplot.png
# :alt: Bar plot of earnings separated by gender.
# :width: 360
# ```
#
# ```{image} images/male_female_earnings_scatterplot.png
# :alt: Scatterplot of earnings separated by gender.
# :width: 340
# ```
#
...
#
# ```{image} images/small_multiples.png 
# :alt: Plot example with the geographic representation of the United States repeated multiple times.
# :width: 600
# ```                             
#
# ### Harnessing Context
...
# %% [markdown] id="496a2b00"
# ### Examples of Bad Visualization
#
# After reviewing many good visualization practices, it is also helpful to examine examples of how improper use of visualization elements can negatively affect interpretation.
#
# ```{image} images/bad_vis1.png
# :alt: Bad visualization showing candidates with party support.
# :width: 600
# ```
#
# In this example, color is used poorly, making it difficult to distinguish and compare the “Yes” category with the other "No" category. In addition, the category ordering is not clearly aligned, which makes it harder for viewers to quickly understand the results.
#
# ```{image} images/bad_vis2.png
# :alt: Bad visualization number of month since the start of 2017 and the highway vehicle miles traveled for all systems.
# :width: 600
# ```
#
# In this example, “Months since the start of 2017” makes it difficult for readers to interpret the timeline. In addition, the scientific notation used on the y-axis is hard to read and may be confusing for a general audience.
#
# ```{image} images/bad_vis3.png
# :alt: Bad visualization showing density of Mother's single years of age.
# :width: 600
# ```
#
# In this example, the data appear to be missing because the histogram bins are set inappropriately, creating the impression that values are missing when this is unlikely to be the case.
