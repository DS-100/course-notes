...
# ### Example: Average Height of UC Berkeley Undergraduates
...
#
# ```{image} images/population_samples.png
# :alt: Illustration of a population, different samples taken from the population, and the variablility in the estimation procedure with a confidence interval shown.
# :width: 650
# ```
#
...
#
# ::: {caution} Why must we resample *with replacement*?
# :class: dropdown
...
# :alt: Illustration of a population, one chosen sample that is then sampled from with replacement to create different bootstrap samples that produce a graph showing the variability in the estimation procedure with a confidence interval shown.
# :width: 700
# ```
#
...
#
# ```{image} images/bootstrapped_samples.png
# :alt: The population and the sampling distribution for the mean is shown first. Then 5 different samples are shown. One sample's bootstrap distribution doesn't contain the population mean at all. The other four samples' bootstrap distributions do contain the population mean with one centered on it, two clearly containing it, and one containing it in one of its tails.
# :width: 600
# :height: 700
# ```
#
...
#
# ::: {note} Confounding factor
# Let T represent a treatment (for example, alcohol use) and Y represent an outcome (for example, lung cancer).
#
# ```{image} images/confounder.png
# :alt:Illustration showing how an outcome (lung cancer) can be influenced by a confounder (smoking) in addition to an exposure (alcohol use). There is a bidirectional arrow between exposure and the counder. Arrows also exist from the exposure to the outcome and from the confounder to the outcome.
# :width: 600
# ```
#
...
#
#
# However, the fact that correlation does not imply causation does not mean that OLS is useless for exploring causation.
#
# Anthropic ran a randomized experiment on whether CS students who did or did not use AI got higher or lower quiz scores. [The link for the Anthropic study can be found here.](https://www.anthropic.com/research/AI-assistance-coding-skills)
#
# While the exact regression analysis conducted by Anthropic researchers is more complex than this, we can think about a simplified model:
#
# $$
# \widehat{\text{Quiz score}} = \hat{\theta}_0 + \hat{\theta}_1 (\text{Used AI})
# $$
#
...
#
# The key difference in this study is that researchers conducted a randomized controlled trial (RCT), in which each participant was randomly assigned into groups that either used AI or did not use AI.
#
# Because the treatment (Used AI) was randomly assigned, it is independent of other factors that affect quiz scores. Therefore, the OLS estimate $\hat{\theta}_1$ can be interpreted as the causal effect of AI usage on quiz scores.
#
...
# :alt: In an experiment, there are no arrows pointing to T (the treatment).
# :width: 600
# ```
#
...
#
# ```{image} images/observational.png
# :alt: In an obersvational study, there could be arrows pointing to T (treatment) and unmeasured cofounders.
# :width: 600
# ```
#
...
#
# ```{image} images/confidence_interval.png 
...
#
# ```{image} images/plover_eggs.jpg
# :alt: A bird sitting on some eggs
# :width: 550
# ```
#
...
# %% id="fcd8d006"
#| fig-alt: A 4 by 4 grid of plots. Along the diagonal, where a variable is paired with itself are histograms showing the distribution of that variable. Outside of the diagonal, where two different variables are paired are scatterplots. Each scatterplot appears to have a clear positive correlation.
import seaborn as sns
sns.pairplot(eggs[["egg_length", "egg_breadth", "egg_weight", 'bird_weight']].to_pandas());

# %% [markdown] id="a9d3016c"
