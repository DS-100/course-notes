...
# - Carry out the procedure of PCA
# - [**Optional for Spring 26**] Dissect Singular Value Decomposition (SVD) and use it to calculate principal components
...
#
# ![Weight (lbs) is plotted against weight (kg) and all datapoints lie along a straight line.](images/dataset3.png)
#
...
#
# ![Weight (lbs) is plotted against weight (kg) and all datapoints lie along a straight line except one datapoint.](images/dataset3_outlier.png)
#
...
#
# ![Three different projections](images/diff_reductions.png)
#
...
#
# ## [Optional for Spring 26] Matrix Decomposition (Factorization)
#
...
#
# ::: {tip} Linear Algebra Review: Matrix Multiplication
...
#
# ![Image illustrating how to perform a dot product](images/matmul2.png)
#
# 2. Each *column* in our output is a *linear transformation* of the original columns based on a column in the transformation matrix 
#
# ![Image illustrating out the output is a linear transformation of the original matrix.](images/matmul3.png)
#
...
#
# ```{image} images/factorization_constraints_1.png 
# :width: 550vw 
# :alt: Factorization constraints
# ```
#
...
#
# ```{image} images/factorization_constraints_2.png
# :width: 550vw
# :alt: But the columns of the resulting matrix are linearly dependent, no matter what the values in the matrix decomposition are! So the rank is less than 3 for any 3x1 * 1x3 decomposition. But original matrix has rank 3! Thus, we are unable to decompose any rank 3 3x3 matrix into a 3x1 * 1x3 decomposition.
# ```
#
...
# :width: 600vw
# :alt: Illustration of principal component analysis. The (large 100x4) original data matrix is shown on the left, multiplied by the principal compoentns (cols) (with dimensions 4x2) to yield latent factors (cols) (with dimensions 100x2).
# ```
#
...
#
# ```{image} images/total_variance_2.png 
# :width: 500vw
# :alt: These latent factors (feature columns) were constructed by a linear combination of features (using PCA). Total variance: 397.15
# ```
#
...
#
# ```{image} images/factorization.png
# :width: 550vw 
# :alt: An image showing the mathematical formulation and matrix visualization of dimensionality reduction via matrix decomposition. At the top, the reconstruction loss function is defined as: L of Z and W equals one over n times the sum from i equals 1 to n of the squared norm of the quantity X sub i minus Z sub i times W. Annotations identify X sub i and the product Z sub i W as row vectors. Below the formula, a diagram illustrates the matrix subtraction: a large green matrix X (n by d) minus the product of a tall yellow matrix Z (n by k) and a wide grey matrix W (k by d). Annotations specify that matrix X is centered (mean subtracted), the rows of Z are the latent vectors used for EDA, and the rows of W are the principal components.
# ```
#
...
#
# ![Takeaways](images/optimization_takeaways.png)
#
# ## [Optional for Spring 26] PCA as Loss Minimization
#
...
#
# ![Illustration of orthonormal vectors](images/orthonormal.png)
#
...
#
# ![Diagonal Matrix](images/diag_matrix.png)
# :::
#
...
#
# ![A dxd matrix where all values are 0 except along the diagonal they are label s sub 1 to s sub r and then the remainder of the diagonal is 0s.](images/s.png)
#
# ### SVD: $V^T$
#
...
#
# ![Illustration of the dxd matrix V](images/v.png)
#
...
#
# ![Mathematical equation for SVD: X = U multiplied by Sigma multiplied by V Transpose](images/svd.png)
#
...
#
# ![Visual represenation of SVD](images/svd_geo.png)
# :::
#
# ### SVD in `NumPy`
#
...
#
# ![matrix X (dimensions n by d) multiplied by V[:, :k] (dimensions d by k) equals matrix Z (dimensions n by k)](images/Z.png)
#
...
# ### PCA Visualization
#
# ![PCA visualization example where the data is first centered and then rotated.](images/rotate_center_plot.png)
#
...
#
# ![Scatterplot with pc1_jittered on the x-axis and pc2_jittered on the y-axis. Datapoints are highlighted in blue for Democrat (on the left of the graph), red for Republican (on the right side of the graph) and orange for Independent (one datapoint in the center).](images/pca_plot.png)
#
# ### Scree Plots
#
...
#
# ![Graph that shows the most amount of variance is explained by the first principal component](images/scree_plot.png)
#
...
#
# ![Example of a bipilot using the data about Democrats, Republicans, and Independents.](images/slide17_2.png)
#
...
# %% [markdown] id="ca0f837f"
# Based on the plot above, it looks like there are two clusters of datapoints. What do you think this corresponds to?
#
...
# :width: 500
# :alt: Example images in the Fashion-MNIST dataset.
# ```
#
...
#
# ![Overview of the process. You start out with a n x d matrix X, do PCA, obtain new features in a n x p matrix and eventually a column vector Y representing the labels.](images/slide21.png) 
#
...
#
# ![Scatterplot of mortality plotted against fertility. Datapoints are shown in blue with vertical red lines connecting certain datpoints to the green predicted fertility line. The red lines are residuals.](images/lin_reg.png)
#
...
#
# ![The same datapoints as above are shown, this time with a dark purple line representing the predicted mortality. Horizontal red lines are drawn from some datapoints to the prediction line.](images/lin_reg_reverse.png)
#
...
#
# ![Again, mortality is plotted against fertility. We see the green predicted fertility line, the purple predicted mortality line, and a black rank 1 approx. line. The lines are all pretty similar, with the green and black lines most similar. The purple line has a greater slope than both the black and green lines.](images/rank1.png)
#
# ### Beyond 1D and 2D
#
