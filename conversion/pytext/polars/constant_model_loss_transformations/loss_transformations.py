...
#
# ```{image} images/slr_modeling.png
# :alt: First, choose a model. Second, choose a loss function. Third, fit the model. And fourth, evaluate the model performance.
# :width: 600
# ```
#
...
#
# $x$ : input / feature / covariate / predictor / regressor (independent variable)
#
# $y$ : output / response / outcome (dependent variable)
#
# $\hat{y}$ : predicted response (estimated value of $y$)
#
# $\theta$ : model parameter(s) / weight(s) / coefficient(s)
#
# $\hat{\theta}$ : estimator, i.e., learned (optimal) parameter values
#
# ## Constant Model + MSE
#
...
# | loss surface           | 2-D ![2D graph of the constant model's loss surface with a singular minimum](images/constant_loss_surface.png)                        | 3-D ![3D graph of simple linear regression's loss surface with the minimum marked](images/slr_loss_surface.png)                                            |
# | loss model             | $\hat{R}(\theta) = \frac{1}{n}\sum^{n}_{i=1} (y_i - \theta_0)^2$ | $\hat{R}(\theta_0, \theta_1) = \frac{1}{n}\sum^{n}_{i=1} (y_i - (\theta_0 + \theta_1 x))^2$ |
# | RMSE                   | 7.72                                                             | 4.31                                                                            |
# | predictions visualized | rug plot ![Rug plot with theta_0_hat marked](images/dugong_rug.png) | scatter plot ![Scatterplot showing the datapoints along with a line representing the predictions.](images/dugong_scatter.png) |
#
...
# %% [markdown] id="a3887799"
# ````{dropdown} Click to see the code
# ```python
# #| fig-alt: The loss surface for the constant model is plotted with the global minimum marked.
# # Constant Model + MSE
# plt.style.use('default') # Revert style to default mpl
...
# %% [markdown] id="2de3e23f"
# ````{dropdown} Click to see the code
# ```python
# #| fig-alt: 3D loss surface with varying values of theta_0 and theta_1. The minimum point on the loss surface is marked
# # SLR + MSE
...
# %% [markdown] id="91298196"
# ````{dropdown} Click to see the code
# ```python
# #| fig-alt: Constant Model Rug Plot
...
# %% [markdown] id="65ff1134"
# ````{dropdown} Click to see the code
# ```python
# #| fig-alt: SLR model scatter plot 
# # In case we're in a weird style state
# sns.set_theme()
# adjust_fontsize(size=16)
# %matplotlib inline
...
# :alt: The graph on the left is the constant model loss with MSE error. The lowest point on the curve is annotated as theta = 25.0. The graph on the right is the constant model with MAE error. The lowest point on the curve is annotated as theta = 22.0.
# ```
#
...
# %% [markdown] id="f34e4a2f"
# ```{image} images/outliers.png
# :alt: On the left, MSE curve is shown with theta=193. On the right, MAE is shown with theata=25.5."
# ```
#
...
#
# ```{image} images/mse_loss_26.png
# :width: 350
# :alt: MSE is shown with theta=26.7
# ```
#
...
#
# ```{image} images/bulge.png
# :alt: Illustration of how to use the tukey-mosteller bulge diagram.
# :width: 600
# ```
#
# Note that:
...
# ````{dropdown} Click to see the code
# ```python
# #| fig-alt: Two scatterplots are shown. On the left, length is plotted against age with the datapoints in blue and showing a positive correlation, although there is some curvature. On the right is the same plot with the addition of a red line that appears centered in the data. Some of the data points are very far from the line.
...
# ````{dropdown} Click to see the code
# ```python
# #| fig-alt: Two scatterplots are shown. On the left, length is plotted against log(Age) with the datapoints in blue and showing a positive correlation. On the right is the same plot with the addition of a red line that appears centered in the data. This time all datapoints appear close to the line.
# z = np.log(y)
#
...
# ````{dropdown} Click to see the code
# ```python
# #| fig-alt: The untransformed variables length and age are plotted against each other along with the prediction curve. The curve follows the points.
