...
# %% [markdown] id="a863c260"
# At the end of the Feature Engineering lecture (Lecture 14), we arrived at the issue of fine-tuning model complexity. We identified that a model that's too complex can lead to overfitting while a model that's too simple can lead to underfitting. This brings us to a natural question: how do we control model complexity to avoid underfitting and overfitting? 
#
...
#
# ```{image} images/simple_under_overfit.png
# :alt: Conceptual illustration of underfitting and overfitting
# :width: 400
# ```
#
...
# :alt: Illustration of how a full dataset is transformed by a train/test split.
# :width: 500
# ```
#
...
#
# ```{image} images/validation_set.png
# :alt: Basic illustration of how the training data can be split into training and validation
# :width: 220
# ```
#
...
#
# ```{image} images/possible_validation_sets.png
# :alt: Four different basic illustrations of how the training data can be split into training and validation; validation data can be taken from any section of the training data.
# :width: 750
# ```
#
...
# :alt: Illustration of cross validation
# :width: 800
# ```
#
...
# :alt: Illustration of cross validation with model selection.
# :width: 800
# ```
#
# ### Hyperparameters
...
# :alt: Illustration of hyperparameter tuning. Annotation says Choose the hyperparameter with lowest CV error. Select alpha = 0.1
# :width: 600
# ```
#
# ### Some Cross-Validation Pitfalls
# If the dataset is sorted, a simple split can produce a validation set that is not representative of the overall data. For example, it might only contain values from one specific range or pattern. To avoid this, the data should be shuffled before splitting so that both training and validation sets reflect the full distribution.
#
# If data points are related, such as paired samples or grouped observations, shuffling can introduce data leakage. This happens when related items appear in both the training and validation sets, leading to overly optimistic performance. To prevent this, related data should be kept together by using group-based splitting.
#
...
# ## Regularization
#
# We've now addressed the first of our two goals for today: creating a framework to assess model performance on unseen data. Now, we'll discuss our second objective: developing a technique to adjust model complexity. This will allow us to directly tackle the issues of underfitting and overfitting.
#
...
#
# $$\hat{\mathbb{Y}} = \theta_0 + \theta_1 x_1 + \theta_2 x_2 + \ldots + \theta_p x_p$$
#
...
#
# What do we mean by a "little bit"? Consider the case where some parameter $\theta_i$ is close to or equal to 0. Then, feature $x_i$ barely impacts the prediction – the feature is weighted by such a small value that its presence doesn't significantly change the value of $\hat{\mathbb{Y}}$. If we restrict how large each parameter $\theta_i$ can be, we restrict how much feature $x_i$ contributes to the model. This has the effect of *reducing* model complexity.
#
...
#
# where $p$ is the total number of parameters in the model. You can think of this as us giving our model a "budget" for how it distributes the magnitudes of each parameter. If the model assigns a large value to some $\theta_i$, it may have to assign a small value to some other $\theta_j$. This has the effect of increasing feature $x_i$'s influence on the predictions while decreasing the influence of feature $x_j$. The model will need to be strategic about how the parameter weights are distributed – ideally, more "important" features will receive greater weighting. 
#
# ::: {note}
...
#
# ```{image} images/constrained_gd.png
# :alt: Loss surface
# :width: 450
# ```
#
...
#
# ```{image} images/diamondreg.png
# :alt: Loss surface with a green diamond overlaid.
# :width: 450
# ```
#
...
#
# When $Q$ is small, we severely restrict the size of our parameters. $\theta_i$s are small in value, and features $x_i$ only contribute a little to the model. The allowed region of model parameters contracts, and the model becomes much simpler:
#
# ```{image} images/diamondpoint.png
# :alt: Loss surface with a green diamond overlaid and a red point chosen within the green diamond.
# :width: 450
# ```
#
# When $Q$ is large, we do not restrict our parameter sizes by much. $\theta_i$s are large in value, and features $x_i$ contribute more to the model. The allowed region of model parameters expands, and the model becomes more complex:
#
# ```{image} images/largerq.png
# :alt: Loss surface with a larger green diamond overlaid and a red point chosen within the green diamond. The point is now closer to the global minimum.
# :width: 450
# ```
#
...
#
# ```{image} images/verylarge.png
# :alt: Loss surface with a very large green diamond overlaid.
# :width: 450
# ```
#
# Now what if $Q$ was extremely small? Most parameters are then set to (essentially) 0. 
#
# * If the model has no intercept term: $\hat{\mathbb{Y}} = (0)x_1 + (0)x_2 + \ldots = 0$.
# * If the model has an intercept term: $\hat{\mathbb{Y}} = \theta_0 + (0)x_1 + (0)x_2 + \ldots = \theta_0$. Remember that the intercept term is excluded from the constraint - this is so we avoid the situation where we always predict 0.
#
# Let's summarize what we have seen. 
#
# ```{image} images/summary.png
# :alt: Conceptual understanding of model complexity, variance, training error, validation error, and under/overfitting.
# :width: 700
# ```
#
...
#
# $$\frac{1}{n} \sum_{i=1}^n (y_i - (\theta_0 + \theta_1 x_{i, 1} + \theta_2 x_{i, 2} + \ldots + \theta_p x_{i, p}))^2 + \lambda \sum_{i=1}^p \vert \theta_i \vert$$ 
...
#
# ```{image} images/green_constrained_gd_sol.png
# :alt: Loss surface
# :width: 400
# ```
#
...
# $$\frac{1}{n} \sum_{i=1}^n (y_i - (\theta_0 + \theta_1 x_{i, 1} + \theta_2 x_{i, 2} + \ldots + \theta_p x_{i, p}))^2 + \lambda \sum_{i=1}^p \theta_i^2$$ 
