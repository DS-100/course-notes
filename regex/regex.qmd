...
The result would be all the characters starting from the leftmost \<div> and the rightmost \<\/div> (inclusive). We can fix this by making our pattern non-greedy, `<div>.*?</div>`. You can read up more on the documentation [here](https://docs.python.org/3/howto/regex.html#greedy-versus-non-greedy).
